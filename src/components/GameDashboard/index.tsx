...
export default function GameDashboard() {
  ...
  const handleSetTeam = (playerIds: string[]) => { 
    if (!room || !playerId || !room.gameStarted) return; 
    socketService.proposeTeam(roomCode, playerIds); 
  };
  ...
  const handleTogglePlayer = (id: string) => {
    ...
    // Use optional chaining and a fallback to prevent "undefined" errors
    const roundIndex = (room.currentRound || 1) - 1;
    const currentReq = MISSION_REQUIREMENTS[roundIndex];
  
    if (!currentReq) return; // Safety check
  ...
  `
    ...
    `}</style>
          ...
          {me?.isGeneral && room.gameStarted && !room.voting?.active && (() => {
            // Get requirements for the current round
            const currentReq = MISSION_REQUIREMENTS[(room.currentRound || 1) - 1];
            ...
            return (
              <div style={{
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          minWidth: "90px",
                          // Subtle grayscale for unselected if we are at capacity
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: "11px", color: isSelectionComplete ? "#c5a059" : "#666" }}>
...
{room.gameStatus === "OVER" && (
  <div className="victory-overlay" style={{
    ...
    <h1 style={{ color: room.winner?.includes('Green') ? '#4caf50' : '#f44336', fontSize: '4rem' }}>
      {room.winner === "Nawabs (Green)" ? "VICTORY" : "DEFEAT"}
    </h1>
    <h2 style={{ color: '#c5a059' }}>{room.winner} controls Bengal.</h2>
    
    {isGameMaster && (
      <button onClick={handleResetGame} style={{ ...primaryBtn, backgroundColor: "#c5a059", color: "#000", padding: "8px 25px", width:"300px" }}>PLAY AGAIN</button>
    )}
  </div>
)}
{
